...
| **js/state.js**        | One state object (frozen or single export). Fields: `cabinetType` ('16:9' | '1:1'), `unit` ('mm' | 'm' | 'ft' | 'in'), `inputs` (which two are active: e.g. `{ aspectRatio, height }`), `values` (numeric values per input, in current unit), `results` (lower/upper configs from calculator), `selectedConfig` (user-confirmed config or null). Export getters/setters or a small API so no other module writes globals.                                                                                                                                                                                                                                                              |
